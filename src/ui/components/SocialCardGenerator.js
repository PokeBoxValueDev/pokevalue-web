...
export async function generateSocialCardCanvas({ boxPrice, totalValue, diff, isProfitable, grade, currencySymbol }) {
    ...
    // 4. Dibujar Logo y Título alineados
    ctx.fillStyle = '#ffffff';
    ctx.textAlign = 'center';
    
    // Si la imagen cargó bien, la dibujamos junto al texto
    if (logoImg.complete && logoImg.naturalHeight !== 0) {
        ctx.drawImage(logoImg, 180, 45, 40, 40);
        ctx.font = '900 32px sans-serif';
        ctx.fillText('PokeBoxValue', 320, 75);
    } else {
        ctx.font = '900 32px sans-serif';
        ctx.fillText('PokeBoxValue', 300, 75);
    }
